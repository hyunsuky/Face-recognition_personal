...
def train(data, label):
	...
	for i in range(n_class):
		...
		# n_same_class = number of items in the same class.
		n_same_class = len(items[-1])

		# n_within_class = Count number of classes with more than single sample.
		if n_same_class > 1:
	...
	# Initialize Su and Se with between-class and within-class matrices, which is alternative to initialize with random data.
	...
	#Start with the M (maximization) step
	for l in range(max_iter):
		#np.pinv calculates Moore-Penrose pseudo-inverse of a matrix.
		F  = np.linalg.pinv(Se) #supp eq. 5
		u  = np.zeros([n_dim, n_class])
		e = np.zeros([n_dim, n_sample])
		pos = 0
		for mi in range(max_per_class + 1):
			if numberBuff[mi]:
				G = -np.dot(np.dot(np.linalg.pinv(mi * Su + Se), Su), F)  #supp eq. 6 : G = −(mS μ + S ε )−1*Su*Se−1

				#The below two formulas are required to compute supp eq. 7
	...
	#M step is followed by E (expectation) step
	# Formula 6.
	F = np.linalg.pinv(Se)
